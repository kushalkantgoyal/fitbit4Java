...
import com.social.fitbit.api.client.model.FacebookError;
import com.social.fitbit.api.client.model.FacebookErrors;
...
class FacebookErrorHandler extends DefaultResponseErrorHandler {
	...
	void handleFacebookError(HttpStatus statusCode, FacebookError error) {
		if (error != null && error.getCode() != null) {
			...
			if (code == FacebookErrors.UNKNOWN) {
				throw new UncategorizedApiException(FACEBOOK_PROVIDER_ID, error.getMessage(), null);
			} else if (code == FacebookErrors.SERVICE) {
				throw new ServerException(FACEBOOK_PROVIDER_ID, error.getMessage());
			} else if (code == FacebookErrors.TOO_MANY_CALLS || code == FacebookErrors.USER_TOO_MANY_CALLS || code == FacebookErrors.EDIT_FEED_TOO_MANY_USER_CALLS || code == FacebookErrors.EDIT_FEED_TOO_MANY_USER_ACTION_CALLS) {
				throw new RateLimitExceededException(FACEBOOK_PROVIDER_ID);
			} else if (code == FacebookErrors.PERMISSION_DENIED || FacebookErrors.isUserPermissionError(code)) {
				throw new InsufficientPermissionException(FACEBOOK_PROVIDER_ID);
			} else if (code == FacebookErrors.PARAM_SESSION_KEY || code == FacebookErrors.PARAM_SIGNATURE) {
				throw new InvalidAuthorizationException(FACEBOOK_PROVIDER_ID, error.getMessage());
			} else if (code == FacebookErrors.PARAM_ACCESS_TOKEN && error.getSubcode() == null) {
				throw new InvalidAuthorizationException(FACEBOOK_PROVIDER_ID, error.getMessage());
			} else if (code == FacebookErrors.PARAM_ACCESS_TOKEN && error.getSubcode() == 463) {
				throw new ExpiredAuthorizationException(FACEBOOK_PROVIDER_ID);
			} else if (code == FacebookErrors.PARAM_ACCESS_TOKEN) {
				throw new RevokedAuthorizationException(FACEBOOK_PROVIDER_ID, error.getMessage());
			} else if (code == FacebookErrors.MESG_DUPLICATE) { 
				throw new DuplicateStatusException(FACEBOOK_PROVIDER_ID, error.getMessage());
			} else if (code == FacebookErrors.DATA_OBJECT_NOT_FOUND || code == FacebookErrors.PATH_UNKNOWN) {
